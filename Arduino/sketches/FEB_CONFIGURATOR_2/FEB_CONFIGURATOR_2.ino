...
uint8_t writeI2C(uint8_t n, char **values){
  if (n > 2){
...
uint8_t readI2C(uint8_t n, char **values){
  uint8_t retval;  
  if (n > 2){
...
char *keywords[MAX_KEYWORDS]= {"i2c_send","i2c_write","i2c_read","scan","write","read","alert","clear_faults","store","read_values"};
  ...
  if (treat_input){
    ...
    if (checkKeyword(command[0])){
      ...
      if (strcmp(command[0],"i2c_send") == 0){
        ...
        Serial.println("i2c send called ok");
      /*---------------------------------------------------------------  
       *   keyword : i2c_write
       ---------------------------------------------------------------*/    
       ...
       *   keyword : writei2c
       ---------------------------------------------------------------*/    
      } else if (strcmp(command[0],"i2c_read") == 0){
        
        Serial.println(readI2C(iw,command));
